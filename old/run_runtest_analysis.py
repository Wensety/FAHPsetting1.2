...
def generate_statistical_report(excel_files, output_file='runtest_statistical_report.xlsx'):
    """執行批次分析並產生統計分析報表"""
    
    print(f"FAHP 統計分析報表生成器 Version {__version__}")
    print(f"將處理 {len(excel_files)} 個文件\n")
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        ...
        # 3. 優先度統計（如果有）
        if all_indicator_data and 'Priority' in ind_df.columns:
...
if __name__ == '__main__':
    ...
    # 生成統計分析報表
    output_file = generate_statistical_report(files, 'runtest_statistical_report.xlsx')
